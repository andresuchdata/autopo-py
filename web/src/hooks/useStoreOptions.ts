...
const normalizeStoreOptions = (items: Array<Record<string, any>> = []): StoreOption[] => {
...
export function useStoreOptions(initialSearch = '') {
  ...
  const optionsRef = useRef<StoreOption[]>([]);
  const optionLookupRef = useRef<Map<number, StoreOption>>(new Map());
  ...
  const fetchOptions = useCallback(
    async ({ searchValue, append }: { searchValue: string; append: boolean }) => {
      ...
      try {
        // Load all stores since API doesn't support pagination
        const response = await poService.getStores(searchValue || undefined);
        ...
        const normalized = normalizeStoreOptions(rawItems);
  ...
  useEffect(() => {
    search(initialSearch).catch((err) => {
      console.error('Initial store fetch failed:', err);
    });
